...
function undo(){const g=state.game;if(!g||!g.undo.length)return;const x=JSON.parse(g.undo.pop());Object.assign(g,x);save();render()}
function newGame(){if(state.game&&!state.game.complete&&!confirm('Nykyinen peli jää kesken. Aloitetaanko uusi?'))return;state.game=null;selectedDice=[];save();render()}

// Returns the highest valid KCD score that can be formed from the selected dice.
// Straights and other combinations may coexist when dice remain, e.g. 1-2-3-4-5-5 = 550.
...
function renderGame(){const g=state.game;if(!g)return;const sorted=[...g.players].sort((a,b)=>b.score-a.score);$('scoreboard').innerHTML='';g.players.forEach((p,i)=>{const rank=sorted.findIndex(x=>x.id===p.id)+1;const d=document.createElement('div');d.className='card p-card '+(i===g.active&&!g.complete?'active ':'')+(g.winners.includes(p.id)?'winner':'');d.style.setProperty('--pc',p.color);d.innerHTML=`<div class="rank">#${rank}</div><h3>${esc(p.name)}</h3><div class="score">${p.score}</div><div class="muted">${Math.max(0,state.target-p.score)} pistettä tavoitteeseen</div>`;$('scoreboard').appendChild(d)});$('activeName').textContent=g.complete?'Peli päättynyt':g.players[g.active].name;$('turnScore').textContent=g.turnScore;$('bankBtn').disabled=g.turnScore<=0||g.complete;$('bustBtn').disabled=g.complete;$('addBtn').disabled=g.complete;$('undoBtn').disabled=!g.undo.length;const wb=$('winnerBanner');if(g.complete){const names=g.players.filter(p=>g.winners.includes(p.id)).map(p=>p.name);wb.classList.remove('hidden');wb.innerHTML=`<h2>🏆 ${esc(names.join(', '))}</h2><div>Peli päättyi. Voittopisteet: ${Math.max(...g.players.map(p=>p.score))}</div>`}else wb.classList.add('hidden');$('turnLog').innerHTML=[...g.turns].reverse().map(t=>`<div class="log-row"><span>#${t.n}</span><span>${esc(t.player)} ${t.bust?'<span class="muted">(bust)</span>':''}</span><strong>${t.points?`+${t.points}`:'0'}</strong></div>`).join('')||'<div class="muted">Ei vielä vuoroja.</div>'}
...
function showStats(){const games=state.history,agg={};games.forEach(g=>g.players.forEach(p=>{agg[p.name]??={games:0,wins:0,points:0};agg[p.name].games++;agg[p.name].points+=p.score;if(g.winners.includes(p.id))agg[p.name].wins++}));$('statsContent').innerHTML=`<div class="stats-grid"><div class="stat"><span class="muted">Pelejä</span><strong>${games.length}</strong></div><div class="stat"><span class="muted">Vuoroja</span><strong>${games.reduce((a,g)=>a+g.turns.length,0)}</strong></div></div><h3 style="margin-top:16px">Pelaajat</h3>${Object.entries(agg).sort((a,b)=>b[1].wins-a[1].wins).map(([n,x])=>`<div class="player-row"><span class="name">${esc(n)}</span><span>${x.wins} voittoa / ${x.games} peliä</span></div>`).join('')||'<p class="muted">Ei vielä valmiita pelejä.</p>'}`;$('statsDialog').showModal()}
...
$('addPlayer').onclick=addPlayer;$('playerName').onkeydown=e=>{if(e.key==='Enter')addPlayer()};$('startGame').onclick=startGame;$('bankBtn').onclick=bank;$('bustBtn').onclick=bust;$('addBtn').onclick=()=>{const v=prompt('Lisättävät pisteet vuoroon:');if(v!==null)addPoints(v)};$('undoBtn').onclick=undo;$('endBtn').onclick=manualEnd;$('newBtn').onclick=newGame;$('rulesBtn').onclick=()=>$('rulesDialog').showModal();$('statsBtn').onclick=showStats;$('clearHistory').onclick=()=>{if(confirm('Poistetaanko koko pelihistoria?')){state.history=[];save();$('statsDialog').close();showStats()}};$('exportBtn').onclick=exportData;$('importFile').onchange=e=>e.target.files[0]&&importData(e.target.files[0]);$('useCalc').onclick=()=>{const v=calcScore(selectedDice);if(v){addPoints(v);clearDice()}};$('clearDice').onclick=clearDice;$('removeDie').onclick=removeLastDie;$('themeBtn').onclick=()=>{state.theme=document.documentElement.classList.contains('dark')?'light':'dark';save();applyTheme()};matchMedia('(prefers-color-scheme:dark)').addEventListener?.('change',()=>{if(state.theme==='system')applyTheme()});
load();